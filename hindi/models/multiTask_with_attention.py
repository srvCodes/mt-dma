...
def write_words_to_file(orig_words, predictions):
	...
	with open(filename, 'w', encoding='utf-8') as f:
		f.write("Words" + '\t\t' + 'Roots' + '\t\t' + "predictions" + '\n')
...
def create_model(X_vocab_len, X_max_len, y_vocab_len, y_max_len, n_phonetic_features, y1, n1, y2, n2, y3, n3, y4, n4, y5, n5, y6, n6,
	...
	x = Bidirectional(RNN(rnn_output_size, kernel_regularizer=l2(0.0001), name='bidirec1'))(x)
...
saved_weights = "./model_weights/frozen_training_weights11.hdf5"
...
if MODE == 'train':
	...
	hist1 = model.fit([X_train, X_decoder_input,  X_right1_tr, X_right2_tr, X_right3_tr, X_right4_tr, X_left1_tr, X_left2_tr, X_left3_tr, X_left4_tr, X_train_phonetics],
				 ...
				 callbacks=[EarlyStopping(patience=12),
								ModelCheckpoint('./model_weights/training_weights11.hdf5', save_best_only=True,
												verbose=1, save_weights_only=False),tb])

	print("Setting weights now.. ")
	model.load_weights('./model_weights/training_weights1.hdf5')
	...
	hist = pseudo_model.fit([X_train, X_decoder_input,  X_right1_tr, X_right2_tr, X_right3_tr, X_right4_tr, X_left1_tr, X_left2_tr, X_left3_tr, X_left4_tr, X_train_phonetics],
				 ...
				 batch_size=BATCH_SIZE, epochs=500,
				 callbacks=[EarlyStopping(patience=12),
				 			ModelCheckpoint('./model_weights/frozen_training_weights11.hdf5	',
												save_best_only=True, verbose=1, save_weights_only=False),
				 			ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=5, min_lr=0.01), 
				 			tb2])
